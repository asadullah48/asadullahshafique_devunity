            ...
            <div className="absolute inset-0 overflow-hidden">
                {particles.map((p, i) => (
                    <motion.div
                        ...
                        initial={{ x: p.x, y: p.y }}
                        animate={{ y: [null, p.yTarget], opacity: [0, 1, 0] }}
                ...
                <div className="flex flex-col-reverse md:flex-row items-center gap-12 max-w-6xl mx-auto">
                    ...
                    <div className="flex-1 text-center md:text-left">
                        ...
                        >
                            ...
                            {/* Description */}
                            <p className="mb-10 text-lg text-zinc-400 md:text-xl max-w-2xl leading-relaxed">
                            ...
                            {/* CTA Buttons */}
                            <div className="flex flex-col items-center md:items-start space-y-4 md:flex-row md:space-x-4 md:space-y-0">
                                <Link href="#projects">
                                ...
                                <Link href="https://github.com/asadullah48" target="_blank" rel="noopener noreferrer">
                                    <Button variant="outline" className="h-12 px-8 border-zinc-700 text-zinc-300 hover:border-[#9CE630] hover:text-[#9CE630] transition-all duration-300">
                                        <Github className="mr-2 h-5 w-5" />
                                        GitHub Profile
                                    </Button>
                                </Link>
                                <a href="/resume.pdf" target="_blank" rel="noopener noreferrer" download="Asadullah_Shafique_Resume_2025.pdf">
                                    <Button variant="outline" className="h-12 px-8 border-[#9CE630]/40 text-[#9CE630] hover:bg-[#9CE630]/10 hover:border-[#9CE630] transition-all duration-300">
                            ...
                            <motion.div
                                ...
                                className="flex items-center justify-center md:justify-start gap-8 mt-16 pt-8 border-t border-zinc-800/50"
                            >
                                {[
                                    { value: "467+", label: "Repos" },
                                    { value: "6", label: "Hackathons" },
                                    { value: "85%", label: "Code Reuse" },
                                    { value: "149+", label: "Tests" },
                                ].map((stat) => (
                    ...
                    {/* RIGHT: photo with hover swap (headshot → vector) */}
                    <div className="flex-shrink-0 relative group cursor-pointer">
                        <div className="relative w-64 h-64 md:w-80 md:h-80 rounded-2xl overflow-hidden border-2 border-[#9CE630]/30 bg-zinc-800">
                            {/* Fallback initials — visible when images fail to load */}
                            <div className="absolute inset-0 flex items-center justify-center text-[#9CE630] text-5xl font-bold select-none z-0">
                                AS
                            </div>
                            {/* Headshot — shown by default, fades out on hover */}
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            <img
                                src="/images/asadullah-headshot.jpg"
                            ...
                            />
                            {/* Vector illustration — hidden by default, shown on hover */}
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            <img
                                src="/images/asadullah-vector.png"
                                alt="Asadullah Shafique illustration"
                        ...
                        <div className="absolute -bottom-3 -right-3 px-3 py-1.5 bg-[#9CE630] text-black text-xs font-bold rounded-full shadow-lg">
                            Open to Collab
                        </div>
                    </div>
