...
const PROJECTS_EN: Project[] = [
  ...
  {
    id: "phase-1-console-todo",
    title: "Phase I Console Todo: Clean Architecture in Pure Python",
    status: "Completed",
    tagline: "Zero runtime dependencies, by design not by accident",
    problem: "Most 'clean architecture' demos import a web framework and an ORM on line one, which quietly makes the architecture untestable in isolation and the claim unfalsifiable. The interesting question is whether the layering actually holds when nothing external is propping it up.",
    solution: "Four layers with one direction of dependency: domain entities that validate themselves, an abstract repository interface, an in-memory implementation behind it, and a service layer holding the business rules. The console is a thin adapter injected at the composition root. Because storage is an interface rather than a database call, the whole application runs on the standard library alone - the dependency list is empty and that is the proof, not the marketing.",
    ...
    description: "A console todo application built to prove that clean architecture holds without a framework: repository pattern, service layer, dependency injection, and an empty dependency list.",
...
const PROJECTS_AR: Project[] = [
  ...
  {
    id: "phase-1-console-todo",
    title: "المرحلة الأولى - تطبيق مهامّ بمعمارية نظيفة ببايثون خالصة",
    status: "Completed",
